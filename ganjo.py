...
async def forward_job(
    ...
    while datetime.now() < end:
        ...
        async for dialog in client.iter_dialogs():
            ...
            # Cek blacklist (cek nama dan username)
            dialog_username = getattr(dialog.entity, 'username', None) or ''
            if dialog_name in blacklisted_groups or f"@{dialog_username}" in blacklisted_groups:
                groups_skipped += 1
                continue
...
@client.on(events.NewMessage(pattern=r'^/setrandombatch\b'))
async def set_random_batch(event):
    if not await require_allowed(event):
        return
    try:
        parts = event.message.raw_text.split()
        if len(parts) < 3:
            return await event.respond("Format: /setrandombatch <min_detik> <max_detik>\nContoh: /setrandombatch 3 10")
        min_val = float(parts[1])
        max_val = float(parts[2])
        if min_val < 0 or max_val < 0 or min_val > max_val:
            return await event.respond("❌ Nilai tidak valid! Pastikan min >= 0 dan max >= min.")
        random_delay_batch_setting[event.sender_id] = {'min': min_val, 'max': max_val}
        save_delay_settings()
        await event.respond(f"✅ Random delay per-batch diset: {min_val} - {max_val} detik")
    except ValueError:
        await event.respond("❌ Parameter harus berupa angka!\nFormat: /setrandombatch <min_detik> <max_detik>")
    except Exception as e:
        logging.error(f"Error pada /setrandombatch: {e}")
        await event.respond(f"Error: {e}")

@client.on(events.NewMessage(pattern=r'^/setrandomgroup\b'))
async def set_random_group(event):
    if not await require_allowed(event):
        return
    try:
        parts = event.message.raw_text.split()
        if len(parts) < 3:
            return await event.respond("Format: /setrandomgroup <min_detik> <max_detik>\nContoh: /setrandomgroup 2 8")
        min_val = float(parts[1])
        max_val = float(parts[2])
        if min_val < 0 or max_val < 0 or min_val > max_val:
            return await event.respond("❌ Nilai tidak valid! Pastikan min >= 0 dan max >= min.")
        random_delay_group_setting[event.sender_id] = {'min': min_val, 'max': max_val}
        save_delay_settings()
        await event.respond(f"✅ Random delay per-grup diset: {min_val} - {max_val} detik")
    except ValueError:
        await event.respond("❌ Parameter harus berupa angka!\nFormat: /setrandomgroup <min_detik> <max_detik>")
    except Exception as e:
        logging.error(f"Error pada /setrandomgroup: {e}")
        await event.respond(f"Error: {e}")

@client.on(events.NewMessage(pattern=r'^/clearrandom\b'))
async def clear_random(event):
    if not await require_allowed(event):
        return
    removed = []
    if event.sender_id in random_delay_batch_setting:
        del random_delay_batch_setting[event.sender_id]
        removed.append("batch")
    if event.sender_id in random_delay_group_setting:
        del random_delay_group_setting[event.sender_id]
        removed.append("group")
    save_delay_settings()
    if removed:
        await event.respond(f"✅ Random delay dihapus untuk: {', '.join(removed)}")
    else:
        await event.respond("Tidak ada random delay yang aktif.")

@client.on(events.NewMessage(pattern=r'^/setmemberfilter\b'))
async def set_member_filter(event):
    if not await require_allowed(event):
        return
    try:
        parts = event.message.raw_text.split()
        if len(parts) < 3:
            return await event.respond(
                "Format: /setmemberfilter <min|-> <max|->\n"
                "Contoh:\n"
                "  /setmemberfilter 100 5000 — grup dengan 100-5000 member\n"
                "  /setmemberfilter 100 - — grup dengan minimal 100 member\n"
                "  /setmemberfilter - 1000 — grup dengan maksimal 1000 member"
            )
        min_str = parts[1].strip()
        max_str = parts[2].strip()
        min_val = None if min_str == '-' else int(min_str)
        max_val = None if max_str == '-' else int(max_str)
        if min_val is not None and max_val is not None and min_val > max_val:
            return await event.respond("❌ Nilai min tidak boleh lebih besar dari max!")
        member_filter_setting[event.sender_id] = {'min': min_val, 'max': max_val}
        save_delay_settings()
        min_display = str(min_val) if min_val is not None else "tanpa batas"
        max_display = str(max_val) if max_val is not None else "tanpa batas"
        await event.respond(f"✅ Filter member diset: min={min_display}, max={max_display}")
    except ValueError:
        await event.respond("❌ Parameter harus berupa angka atau tanda '-'!\nFormat: /setmemberfilter <min|-> <max|->")
    except Exception as e:
        logging.error(f"Error pada /setmemberfilter: {e}")
        await event.respond(f"Error: {e}")

@client.on(events.NewMessage(pattern=r'^/clearmemberfilter\b'))
async def clear_member_filter(event):
    if not await require_allowed(event):
        return
    if event.sender_id in member_filter_setting:
        del member_filter_setting[event.sender_id]
        save_delay_settings()
        await event.respond("✅ Filter member berhasil dihapus!")
    else:
        await event.respond("Tidak ada filter member yang aktif.")
...
async def add_blacklist(event):
    ...
    try:
        ...
        if not nama:
            return await event.respond("Format salah. Gunakan:\n/blacklist_add <nama grup>\n/blacklist_add @username_grup")
        # Jika input berupa @username, resolve ke nama grup
        if nama.startswith('@'):
            try:
                entity = await client.get_entity(nama)
                group_name = getattr(entity, 'title', None) or nama
                group_username = getattr(entity, 'username', None)
                # Simpan sebagai @username agar konsisten
                blacklisted_groups.add(f"@{group_username}" if group_username else group_name)
                save_blacklist()
                display = f"@{group_username}" if group_username else group_name
                await event.respond(f"'{group_name}' ({display}) berhasil masuk ke blacklist!")
            except Exception as e:
                logging.error(f"Error resolve username {nama}: {e}")
                # Tetap simpan @username meskipun gagal resolve
                blacklisted_groups.add(nama)
                save_blacklist()
                await event.respond(f"'{nama}' berhasil masuk ke blacklist! (username tidak bisa di-resolve)")
        else:
            blacklisted_groups.add(nama)
            save_blacklist()
...
async def remove_blacklist(event):
    ...
    try:
        nama = " ".join(event.message.raw_text.split()[1:]).strip()
        if not nama:
            return await event.respond("Format salah. Gunakan:\n/blacklist_remove <nama grup>\n/blacklist_remove @username_grup")
    ...
    try:
        ...
        async for dialog in client.iter_dialogs(limit=100):  # Limit untuk tidak terlalu lama
            ...
            try:
                ...
                d_username = getattr(dialog.entity, 'username', None) or ''
                is_bl = dialog_name in blacklisted_groups or f"@{d_username}" in blacklisted_groups
                if dialog.is_group:
                    total_grup += 1
                    is_blacklisted = "❌ BLACKLIST" if is_bl else "✅"
...
============================
5. Blacklist Grup
 — /blacklist_add @usernamegrup — Tambahkan grup ke blacklist
 — /blacklist_remove @usernamegrup — Hapus grup dari blacklist
...
Selamat mencoba dan semoga hari-harimu penuh cinta! Kalau masih ada yang bingung bisa chat pengembangku/kirimkan feedback ya!"""
    ...
    await event.respond(teks1)
    await event.respond(teks2)
...
async def info_handler(event):
    ...
    text = (
        ...
        "Panduan: /help\n"
        "Statistik: /stats\n"
        "Hubungi pembuat: https://t.me/nDesis"
    )
...
@client.on(events.NewMessage(pattern=r'^/previewgrup\b'))
async def preview_grup(event):
    if not await require_allowed(event):
        return
    try:
        parts = event.message.raw_text.split()
        ...
        async for dialog in client.iter_dialogs():
            ...
            try:
                ...
                if dialog.is_group:
                    ...
                    if hasattr(dialog.entity, 'megagroup') and dialog.entity.megagroup:
                        is_group_type = True
                    elif not getattr(dialog.entity, 'broadcast', False):
                        is_group_type = True
                elif isinstance(dialog.entity, Chat):
                    is_group_type = True
            except Exception:
            ...
            # Cek whitelist
            if use_whitelist and dialog_name not in whitelisted_groups:
                continue
            ...
            if min_member is not None or max_member is not None:
                ...
                if not is_in_member_range(count, min_member, max_member):
                    skipped += 1
                    continue
                grup_list.append(f"✅ {dialog_name} ({count or '?'} member)")
            else:
                grup_list.append(f"✅ {dialog_name}")

        teks = f"== Preview Target Grup (max {limit}) ==\n\n"
        if not grup_list:
            teks += "Tidak ada grup yang memenuhi kriteria."
        else:
            for idx, g in enumerate(grup_list, 1):
                teks += f"{idx}. {g}\n"
            teks += f"\nTotal: {len(grup_list)} grup target"
            if skipped:
                teks += f" | {skipped} di-skip (blacklist/filter)"

        await event.respond(teks)
    except Exception as e:
        logging.error(f"Error pada /previewgrup: {e}")
        await event.respond(f"Error: {e}")

@client.on(events.NewMessage(pattern=r'^/listaktif\b'))
async def list_aktif(event):
    if not await require_allowed(event):
        return
    try:
        parts = event.message.raw_text.split()
        jam = int(parts[1]) if len(parts) > 1 else 24
        limit = int(parts[2]) if len(parts) > 2 else 20

        cutoff = datetime.now() - timedelta(hours=jam)
        aktif_list = []

        async for dialog in client.iter_dialogs():
            if len(aktif_list) >= limit:
                break

            is_group_type = False
            try:
                from telethon.tl.types import Channel, Chat
                if dialog.is_group or isinstance(dialog.entity, Chat):
                    is_group_type = True
                elif isinstance(dialog.entity, Channel):
                    if hasattr(dialog.entity, 'megagroup') and dialog.entity.megagroup:
                        is_group_type = True
            ...
            if dialog.date and dialog.date.replace(tzinfo=None) >= cutoff:
                nama = dialog.name or "Tanpa Nama"
                waktu = dialog.date.strftime('%d/%m %H:%M')
                aktif_list.append(f"{nama} — terakhir aktif: {waktu}")

        teks = f"== Grup Aktif (dalam {jam} jam terakhir) ==\n\n"
        if not aktif_list:
            teks += "Tidak ada grup yang aktif dalam periode tersebut."
        else:
            for idx, g in enumerate(aktif_list, 1):
                teks += f"{idx}. {g}\n"
            teks += f"\nTotal: {len(aktif_list)} grup aktif"

        await event.respond(teks)
    except Exception as e:
        logging.error(f"Error pada /listaktif: {e}")
        await event.respond(f"Error: {e}")

@client.on(events.NewMessage(pattern=r'^/failedgrup\b'))
async def failed_grup(event):
    if not await require_allowed(event):
        return
    user_id = event.sender_id
    session = forward_sessions.get(user_id)
    ...
    if not session:
        return await event.respond("Belum ada session forward yang tercatat.")

    failed = session.get('failed_groups', [])
    status = session.get('status', 'unknown')
    sent = session.get('sent', 0)
    fail_count = session.get('failed', 0)

    teks = f"== Grup Gagal (Session Terakhir) ==\n\n"
    teks += f"Status: {status}\n"
    teks += f"Terkirim: {sent} | Gagal: {fail_count}\n\n"

    if not failed:
        teks += "Tidak ada grup yang gagal! 🎉"
    else:
        ...
        for idx, nama in enumerate(unique_failed, 1):
            teks += f"{idx}. {nama}\n"
        teks += f"\nTotal: {len(unique_failed)} grup gagal"

    await event.respond(teks)

@client.on(events.NewMessage(pattern=r'^/cekresiko\b'))
async def cek_resiko(event):
    if not await require_allowed(event):
        return
    try:
        parts = event.message.raw_text.split()
        if len(parts) < 4:
            return await event.respond(
                "Format: /cekresiko <jumlah_grup> <jeda_grup> <jeda_batch>\n"
                "Contoh: /cekresiko 50 3 10"
            )
        jumlah_grup = int(parts[1])
        jeda_grup = float(parts[2])
        jeda_batch = float(parts[3])

        # Hitung estimasi waktu per batch
        waktu_per_batch = jumlah_grup * (1 + jeda_grup) + jeda_batch
        pesan_per_menit = (jumlah_grup / waktu_per_batch) * 60 if waktu_per_batch > 0 else 999

        # Heuristik risiko
        if pesan_per_menit > 30:
            level = "🔴 TINGGI"
            saran = "Sangat berisiko kena flood! Naikkan jeda antar grup dan batch."
        elif pesan_per_menit > 15:
            level = "🟡 SEDANG"
            saran = "Ada risiko flood. Pertimbangkan naikkan jeda sedikit."
        elif pesan_per_menit > 8:
            level = "🟢 RENDAH"
            saran = "Cukup aman, tapi tetap pantau rate limit."
        else:
            level = "✅ AMAN"
            saran = "Konfigurasi ini tergolong aman dari flood."

        teks = (
            f"== Analisis Risiko Flood ==\n\n"
            f"Jumlah grup: {jumlah_grup}\n"
            f"Jeda antar grup: {jeda_grup} detik\n"
            f"Jeda antar batch: {jeda_batch} detik\n\n"
            f"Estimasi waktu per batch: {waktu_per_batch:.1f} detik\n"
            f"Kecepatan kirim: ~{pesan_per_menit:.1f} pesan/menit\n\n"
            f"Tingkat risiko: {level}\n"
            f"Saran: {saran}"
        )
        await event.respond(teks)
    except ValueError:
        ...
        logging.error(f"Error pada /cekresiko: {e}")
        await event.respond(f"Error: {e}")

@client.on(events.NewMessage(pattern=r'^/join\b'))
async def join_cmd(event):
    ...
    try:
        parts = event.message.raw_text.split()
        if len(parts) < 2:
            return await event.respond("Format: /join <@username atau link invite>\nContoh: /join @namachannel")
        target = parts[1].strip()
        try:
            entity = await client.get_entity(target)
            await client(JoinChannelRequest(entity))
            nama = getattr(entity, 'title', target)
            await event.respond(f"✅ Berhasil join ke: {nama}")
        except Exception as e:
            await event.respond(f"❌ Gagal join: {e}")
    except Exception as e:
        logging.error(f"Error pada /join: {e}")
        await event.respond(f"Error: {e}")

@client.on(events.NewMessage(pattern=r'^/leave\b'))
async def leave_cmd(event):
    ...
    try:
        parts = event.message.raw_text.split()
        if len(parts) < 2:
            return await event.respond("Format: /leave <@username>\nContoh: /leave @namachannel")
        target = parts[1].strip()
        try:
            entity = await client.get_entity(target)
            await client(LeaveChannelRequest(entity))
            nama = getattr(entity, 'title', target)
            await event.respond(f"✅ Berhasil leave dari: {nama}")
        except Exception as e:
